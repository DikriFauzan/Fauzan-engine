...
## Integrates with SharedWorldState and modular converter system
...
func _run_import():
    ...
    if not sws_node:
        push_error("NeoEngineImporter: SharedWorldState not found in root. Cannot import.")
        _importers_active = false
        return

    var plan = sws_node.get_data("last_scene_plan", {})
    if typeof(plan) != TYPE_DICTIONARY or plan.is_empty():
        push_error("NeoEngineImporter: No valid scene_plan found in SharedWorldState.")
        _importers_active = false
        return
